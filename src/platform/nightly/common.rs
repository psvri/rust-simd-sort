...
pub trait SimdCompare<T: SimdSortable, const NUM_LANES: usize>: Copy + Debug {
    ...
    fn reducemin(mask: Self) -> T;
    fn reducemax(mask: Self) -> T;
...
/*fn partition_vec<T: SimdSortable, const N: usize, U: SimdCompare<T, N>>(
    ...
    let gt_mask = U::ge(*curr_vec, *pivot_vec);
    let amount_gt_pivot = U::ones_count(gt_mask);
    println!("mask: {:?}", gt_mask);
    println!("curr_vec: {:?}", curr_vec);
    println!("amount_gt_pivot: {:?}", amount_gt_pivot);
    println!("left: {:?}", left);
    println!("right: {:?}", right);
    //println!("mask is {:?}", gt_mask);
    //println!("not mask is {:?}", U::not_mask(gt_mask));
    println!("before left {:?}", &data[left..left + 8]);
    U::mask_compressstoreu(&mut data[left..], U::not_mask(gt_mask), *curr_vec);
    println!("after left {:?}", &data[left..left + 8]);
    println!(
        "before right {:?}",
        &data[(right - amount_gt_pivot)..(right - amount_gt_pivot + 8)]
    );
    U::mask_compressstoreu(&mut data[(right - amount_gt_pivot)..], gt_mask, *curr_vec);
    println!(
        "after right {:?}",
